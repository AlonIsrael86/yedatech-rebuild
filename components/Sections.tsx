...
/* ── Integrations ────────────────────────────────────────────────────── */
// Real brand marks (lucide dropped brand icons) — simple-icons paths, viewBox 0 0 24 24.
const INTEGRATION_BRANDS: Record<string, { d: string; color: string }> = {
  Zoom: {
    color: "#0B5CFF",
...
const INTEGRATION_LUCIDE: Record<string, keyof typeof Icons> = {
  "CRM synchronization": "RefreshCw",
  "Single sign-on & user provisioning": "KeyRound",
  "HR & enterprise system connections": "Building2",
};
function IntegrationIcon({ name }: { name: string }) {
  const brand = INTEGRATION_BRANDS[name];
  if (brand)
    return (
      <svg viewBox="0 0 24 24" className="size-4 shrink-0" fill={brand.color} aria-hidden>
        <path d={brand.d} />
      </svg>
    );
  const key = INTEGRATION_LUCIDE[name];
  if (key) {
    const L = Icons[key] as Icons.LucideIcon;
    return <L className="size-4 shrink-0 text-royal" aria-hidden />;
  }
  return null;
}
              ...
              <div className="h-full rounded-[var(--radius-card)] border border-line bg-white p-6 shadow-[var(--shadow-card)]">
                ...
                <ul className="mt-4 flex flex-wrap gap-2">
                  {g.items.map((it) => (
                    <li
                      key={it}
                      className="inline-flex items-center gap-2 rounded-[var(--radius-pill)] border border-line bg-canvas px-3 py-1.5 text-[14px] font-medium text-ink-soft"
                    >
                      <IntegrationIcon name={it} />
                      {it}
                    </li>
                  ))}
                </ul>
